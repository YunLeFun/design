...
YunLeFun Design 当前可作为 Web 产品的设计基础试用：共享 token、明暗主题、Vue 组件与源码分发已经接通。当前文档展示开发中的源码；正式公共版本仍需经过真实应用接入和发布验收。
...
| 基础分发 | CSS/SCSS 同源构建，Vue/Nuxt 接入，Registry 同源生成          | 发布版本、变更记录与线上产物验收                       |
| 组件行为 | Reka UI 负责复杂行为；已有 Dialog、Demo 和分发测试           | 对全部组件逐项完成键盘、焦点、可访问名称与错误状态审查 |
| 使用范围 | 文档站复用真实组件，提供多尺寸与明暗预览                     | 主站和另一个真实应用分别完成接入验收                   |
